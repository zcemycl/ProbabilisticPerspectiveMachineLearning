...
while ~out
    ...
    for i=1:size(XTrainshuffle,4)/args.batch_size
        ...
        XBatch=gpdl(XTrainshuffle(:,:,:,idx));

        [GradDis,GradGen] = ...
            dlfeval(@modelGradients,XBatch,genNoiseCDC(args),...
            paramsDis,paramsGen,args);
...
%% Discriminator
function dly = Discriminator(dlx,params,args)
% Convolution group
...
%% Progress Plot
function progressplot(args,paramsGen)
...
fake_data = gpdl(cat(1,fixednoise,cc,dc),'CB');
fake_images = extractdata(Generator(fake_data,paramsGen));
...
function [d_loss,g_loss] = reportprogress(x,z,paramsDis,...
    paramsGen,args)
fake_images = Generator(z,paramsGen);
d_output_real = Discriminator(x,paramsDis,args);
d_output_fake = Discriminator(fake_images,paramsDis,args);
...
%% Model Gradients
function [GradDis,GradGen] = modelGradients(x,z,paramsDis,...
    paramsGen,args)
fake_images = Generator(z,paramsGen);
d_output_real = Discriminator(x,paramsDis,args);
d_output_fake = Discriminator(fake_images,paramsDis,args);
